...
export class Financials extends Component {
    ...
    componentDidMount(){
        axios.get('/api/v1/fetchPayments')
        .then(res =>{
            this.setState({
    ...
    updatePayment = (e) => {
        ...
        axios.post('/api/v1/updatePayment', data)
            .catch(err=>console.log('err', err))
            .then(res=>{
                if(res.data.success){
                    this.setState({ payments: this.state.payments.map(x => x.id === parseInt(res.data.data.id) ? x= res.data.data :x ) })
                }
    ...
    render() {
        ...
        const renderItems =  this.state.payments.filter((i)=>{ if(this.state.search == null) return i; else if(i.business.toLowerCase().includes(this.state.search.toLowerCase()) || i.type.toLowerCase().includes(this.state.search.toLowerCase()) || i.fromTo.toLowerCase().includes(this.state.search.toLowerCase()) || i.purpose.toLowerCase().includes(this.state.search.toLowerCase()) ){ return i }}).slice(indexOfFirstItem, indexOfLastItem).map((i, index) => {
            return (
                <tr key={index}>
                    <td>{index+1}</td>
                    <td>{i.business}</td>
                    ...
                    <td>
                        <strong>Purpose:</strong> {i.purpose}<br/>
                        <strong>Remarks:</strong><section className="not-found-controller" dangerouslySetInnerHTML={{ __html: i.remarks }}/></td>
                    <td><img src="/images/icons/edit.svg" style={{maxWidth: '20px'}} onClick={()=>this.editModalOn(i)}/></td>
                </tr>
        )})
        const pageNumbers = []
        for (let i = 1; i <= Math.ceil(this.state.payments.length / itemsPerPage); i++) { pageNumbers.push(i) }
        ...
        return (
            <>
                ...
                <div className="flex-h">
                    ...
                    <ModalBody>
                        <form encType="multipart/form-data" onSubmit={this.updatePayment}>
                            <div className="row">
